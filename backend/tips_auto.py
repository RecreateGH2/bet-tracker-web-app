...
def _post_matches_meeting(text: str, date_patterns: List[re.Pattern]) -> bool:
    if not text:
        return False
    has_date = any(p.search(text) for p in date_patterns)
    has_venue = ("沙田" in text) or ("跑馬地" in text)
    has_topic = any(kw in text for kw in ("賽馬", "跑馬", "賽馬日", "馬王", "貼士", "場"))
    # Date + (venue or topic keyword) — venue is the strongest signal but
    # some tipsters only mention 賽馬 in their post text.
    return has_date and (has_venue or has_topic)
...
async def run_auto_fetch(meeting_date: Optional[str] = None) -> Dict[str, dict]:
    """For each configured handle, scrape recent posts, keep those matching
    the meeting, download attached images, and trigger Poe extraction.
    Returns a per-handle summary dict for logging / the UI."""
    ...
    log.info(
        f"auto-fetch: scanning {len(handles)} handles for meeting {meeting_date} "
    ...
    for idx, handle in enumerate(handles):
        try:
            handle_summary = await _process_handle(handle, meeting_date, date_patterns)
...
async def _process_handle(handle: str, meeting_date: str, date_patterns: List[re.Pattern]) -> dict:
    ...
    for post in posts:
        if not _post_recent(post.get("timestamp")):
            continue
        if not _post_matches_meeting(post.get("text", ""), date_patterns):
            continue
        matched_count += 1
